...
namespace GoOrCs.SharedKernel.Caching.Distributed;
...
public class DistributedRedisCache : IDistributedRedisCache
{
    ...
    public async Task<bool> ExistsAsync(string key)
    {
        ...
        return await RunWithPolicyAsync(async () => await _cache.KeyExistsAsync(GetKeyForRedisCache(key)));
    }
    ...
    public async Task<bool> SetStringAsync(string key, string value, TimeSpan? expiry = null, bool keepTtl = false)
    {
        ...
        return await RunWithPolicyAsync(async () =>
            await _cache.StringSetAsync(GetKeyForRedisCache(key), Encoding.UTF8.GetBytes(value), absoluteExpireTime, keepTtl));
    }
    ...
    public async Task<bool> DeleteAsync(string key)
    {
        ...
        return await RunWithPolicyAsync(async () => await _cache.KeyDeleteAsync(GetKeyForRedisCache(key)));
    }
    ...
    public async Task<bool> DeleteByPatternAsync(string pattern)
    {
        ...
        var keys = _server.Keys(pattern: GetKeyForRedisCache(pattern)).ToArray();
    ...
    public async Task<string> GetStringAsync(string key)
    {
        ...
        return await RunWithPolicyAsync(async () =>
        {
            var value = await _cache.StringGetAsync(GetKeyForRedisCache(key));
